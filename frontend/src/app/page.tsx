...
const steps = [
  { num: "1", title: "Sign Up", desc: "Create your free account in seconds. No credit card required." },
  { num: "2", title: "Connect & Upload", desc: "Upload documents, connect your AI provider, and import your tasks." },
  { num: "3", title: "Work Smarter", desc: "Let AI handle the heavy lifting while you focus on what matters." },
];

const techStack = [
  { name: "Next.js", desc: "React Framework", icon: "⚛️" },
  { name: "FastAPI", desc: "Python Backend", icon: "⚡" },
  { name: "PostgreSQL", desc: "Database + pgvector", icon: "🗄️" },
  { name: "Gemini", desc: "Google AI", icon: "🧠" },
  { name: "OpenRouter", desc: "Multi-Model", icon: "🔄" },
  { name: "Ollama", desc: "Local AI", icon: "🦙" },
];
    ...
    <div className="min-h-screen">
      ...
      <section className="relative grid-bg overflow-hidden pt-32 pb-24 md:pt-40 md:pb-32">
        ...
        <div className="relative mx-auto max-w-3xl px-6 text-center">
          ...
          <h1 className="fade-up text-4xl font-bold leading-[1.1] tracking-tight sm:text-5xl md:text-6xl" style={{ animationDelay: "0.1s" }}>
            Your AI-Powered<br /><span className="grad">Productivity Hub</span>
          </h1>
          ...
          <div className="fade-up mt-10 flex flex-col items-center justify-center gap-3 sm:flex-row" style={{ animationDelay: "0.3s" }}>
            <a href="/dashboard" className="btn-primary rounded-full px-7 py-3 text-sm font-medium">Open App →</a>
            <a href="#features" className="btn-ghost rounded-full px-7 py-3 text-sm font-medium">See Features</a>
          </div>
        </div>
      </section>
      ...
      <section id="features" className="py-24 md:py-32">
        <div className="mx-auto max-w-6xl px-6">
          <div className="mb-16 text-center">
            <p className="mb-3 text-xs font-semibold uppercase tracking-widest text-[var(--accent)]">Features</p>
            ...
            <p className="mx-auto mt-4 max-w-lg text-sm leading-relaxed text-[var(--text-dim)]">
              Six powerful modules that work together seamlessly. Replace your entire productivity stack.
            </p>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {features.map(f => (
              <div key={f.title} className="glass group rounded-2xl p-6 transition-all duration-200 hover:translate-y-[-2px]">
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="rounded-full bg-[var(--accent)]/10 px-2 py-0.5 text-[10px] font-medium text-[var(--accent)]">{f.tag}</span>
                </div>
                <p className="text-[13px] leading-relaxed text-[var(--text-dim)]">{f.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section id="how" className="border-y border-[var(--border)] bg-white py-24 md:py-32">
        <div className="mx-auto max-w-5xl px-6">
          <div className="mb-16 text-center">
            <p className="mb-3 text-xs font-semibold uppercase tracking-widest text-[var(--accent)]">How It Works</p>
            <h2 className="text-3xl font-bold tracking-tight md:text-4xl">Get Started in Minutes</h2>
          </div>
          <div className="grid gap-12 md:grid-cols-3 md:gap-8">
            {steps.map(s => (
              <div key={s.num} className="text-center">
                <div className="mx-auto mb-5 flex h-12 w-12 items-center justify-center rounded-full border border-[var(--border)] bg-[var(--bg)] text-sm font-bold grad shadow-sm">{s.num}</div>
                <h3 className="mb-2 text-base font-semibold">{s.title}</h3>
                <p className="text-sm leading-relaxed text-[var(--text-dim)]">{s.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 md:py-32">
        <div className="mx-auto max-w-5xl px-6">
          <div className="mb-16 text-center">
            <p className="mb-3 text-xs font-semibold uppercase tracking-widest text-[var(--accent)]">Tech Stack</p>
            <h2 className="text-3xl font-bold tracking-tight md:text-4xl">Built with Modern Tech</h2>
          </div>
          <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-6">
            {techStack.map(t => (
              <div key={t.name} className="glass rounded-xl p-4 text-center transition-all hover:translate-y-[-2px]">
                <div className="mb-2 text-2xl">{t.icon}</div>
                <p className="text-sm font-semibold">{t.name}</p>
                <p className="text-[11px] text-[var(--text-dim)]">{t.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* CTA */}
      <section className="border-y border-[var(--border)] bg-white py-24 md:py-32">
        <div className="mx-auto max-w-2xl px-6 text-center">
          <h2 className="text-3xl font-bold tracking-tight md:text-4xl">Ready to Work Smarter?</h2>
          <p className="mx-auto mt-4 max-w-md text-sm leading-relaxed text-[var(--text-dim)]">
            Join thousands of users who have transformed their workflow with Corely AI.
          </p>
          <div className="mt-10">
            <a href="/dashboard" className="btn-primary inline-block rounded-full px-8 py-3.5 text-sm font-medium">Open App →</a>
          </div>
        </div>
      </section>
